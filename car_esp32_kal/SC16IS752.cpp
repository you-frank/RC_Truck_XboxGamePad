/* 
SC16IS752 driver for Arduino  (I2C-2xUART Module)
*/
...
bool SC16IS752::write(uint8_t channel, uint8_t val)
{
  return WriteByte(channel, val);
}
...
bool SC16IS752::WriteByte(uint8_t channel, uint8_t val)
{
  uint8_t tmp_lsr;
...
int SC16IS752::readBytes(uint8_t channel, uint8_t *buffer)
{
  int i;
  
  for(i=0; i<ReadRegister(channel, SC16IS750_REG_RXLVL); i++){
    buffer[i] = ReadRegister(channel, SC16IS750_REG_RHR);
  }
  return i;
}
/*
...
// GPS 수신용으로 만든것임. buffer size 64를 넘으면 무조건 -1을 return할것임.
int SC16IS752::readLine(uint8_t channel, char *buffer)
{
  uint32_t time_stamp;
  time_stamp = millis();
  if(millis()-readline_timeout > 1000 || readline_count>64){
    readline_count=0;
    readline_timeout=time_stamp;
    return -1; // timeout or bigger than buffer size(64)
  }

  for(int i=0; i<ReadRegister(channel, SC16IS750_REG_RXLVL); i++){
    if(readline_count==0){
      readline_timeout=time_stamp;
    }
    
    buffer[readline_count] = (char)ReadRegister(channel, SC16IS750_REG_RHR);
    if(readline_count>2 && buffer[readline_count-1]=='\r' && buffer[readline_count]=='\n'){
      int buflen=readline_count-1;
      readline_count=0;
      buffer[buflen]='\0';
      return buflen;
    }
...
void SC16IS752::flush(uint8_t channel)
{
  ...
  do {
    tmp_lsr = ReadRegister(channel, SC16IS750_REG_LSR); // Line Status Register
  } while ((tmp_lsr&0x20) ==0);
