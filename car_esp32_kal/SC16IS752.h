/* 
  ...
  Modified by Frank you :
    Add readBytes() for Channel 2
    Make readBytes size 64 because SC16IS752 Que Size is 64byte. 
    When pySerial transmits a 12-byte frame, 
    it is divided into 1,2,3,3,2,1 at 9600bps transmission,
    it is divided into 6,3,2,1 at 115200bps transmission,
*/
...
{ 
  public:
    ...
    bool    write(uint8_t channel, uint8_t val);
    ...
    int readBytes(uint8_t channel, uint8_t *buffer);
    int readLine(uint8_t  channel, char *buffer);
  ...
  private:
    uint8_t  readline_count;
    unsigned long readline_timeout;
    ...
    bool    WriteByte(uint8_t channel, uint8_t val);
